...
- `docker compose ps`
- conectividade da aplicação até o MongoDB configurado
- conectividade da aplicação até o RabbitMQ configurado
- logs da API para identificar qual check falhou
...
```bash
kubectl logs -n service-order -l app=service-order-execution-service --tail=100
...
- `MONGODB_URL` inválida ou inacessível
- `RABBITMQ_URL` inválida ou inacessível
...
```bash
kubectl describe pod <pod-name> -n service-order
docker pull ghcr.io/<owner>/service-order-execution-service:sha-<commit>
...
kubectl port-forward -n service-order svc/service-order-execution-service 8003:80
...
```bash
kubectl logs deployment/service-order-execution-service -n service-order --tail=100
...
- `EC2_SSH_PRIVATE_KEY`
- `APP_ENV_PROD`
- existência do diretório `/opt/service-order-execution-service` na EC2
...
- validar backups e retenção do MongoDB
