...
```bash
kubectl logs -n service-order deployment/service-order-execution-service --tail=50
```

Then confirm in Datadog Logs that `service:service-order-execution-service` entries include
...
kubectl describe hpa service-order-execution-service -n service-order
```
