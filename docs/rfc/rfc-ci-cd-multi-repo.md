...
- `service-order-execution-service`: FastAPI application, Docker image, k8s manifests and
...
`service-order-execution-service` workflow must not package, deploy or validate Terraform for
that Lambda.
