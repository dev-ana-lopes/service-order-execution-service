...
`service-order-execution-service` running on k3s.
...
- `service-order-execution-service` validates customer JWTs with the same
  `CUSTOMER_JWT_SECRET`.

Admin authentication:

- Admin JWTs remain owned by `service-order-execution-service`.
