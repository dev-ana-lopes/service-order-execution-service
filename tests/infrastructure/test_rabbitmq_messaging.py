...
def test_rabbitmq_publisher_serializes_valid_event_message() -> None:
    ...
    publisher = RabbitMqEventPublisher(
        ...
        RabbitMqEventPublisherSettings(
            ...
            routing_key="service-order.execution",
        ),
    )
    ...
    assert published["routing_key"] == "service-order.execution"
    assert body["event_type"] == "EXECUTION_QUEUED"
    assert body["payload"] == {"execution_id": "execution-1"}
...
def test_blocking_rabbitmq_publisher_tracks_and_drains_published_messages(
    ...
    assert messages[0]["event_type"] == "EXECUTION_QUEUED"
